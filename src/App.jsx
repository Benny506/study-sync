 ...
 * Lobby Component
 */
const Lobby = ({ roomID, setRoomID, userName, setUserName, onJoin }) => (
  <motion.div 
  ...
  >
    <div className="text-center mb-5">
      <div className="d-inline-block p-4 glass-card mb-4 pulse">
        <Share2 size={48} className="text-cyan glow-text-cyan" />
      </div>
    ...
    <Card className="glass-card p-4 p-md-5 w-100" style={{ maxWidth: '480px' }}>
      <h4 className="fw-bold mb-4 d-flex align-items-center gap-2">
        <Terminal size={20} className="text-cyan" /> Configure Connection
      </h4>
      <Form onSubmit={onJoin}>
        ...
        <Button type="submit" className="btn-prism w-100 py-3 d-flex align-items-center justify-content-center gap-2">
          Initialize Session <Zap size={18} />
        </Button>
      </Form>
    </Card>
  </motion.div>
      ...
      <Col>
        <div className="d-flex align-items-center gap-3">
          <div className="p-3 glass-card">
            <Share2 size={24} className="text-cyan" />
          </div>
          <div>
      ...
      <Col lg={3}>
        <Card className="glass-card p-4 mb-4 h-100 shadow-lg">
          ...
          <div className="d-flex flex-column gap-3">
            <div className="d-flex align-items-center gap-3 p-3 bg-white bg-opacity-5 rounded-4 border border-white border-opacity-10 shadow-sm">
            ...
            {peers.map(p => {
              ...
              return (
                <motion.div key={p.sessionID} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="d-flex align-items-center gap-3 p-3 bg-white bg-opacity-5 rounded-4 border border-white border-opacity-5 shadow-sm">
                  ...
                  <div className="flex-grow-1 overflow-hidden">
                    ...
                    {isNameDuplicate && <div className="smaller mono text-secondary opacity-50" style={{fontSize: '0.6rem'}}>Session: #{p.sessionID.substring(0, 4)}</div>}
                  </div>
                </motion.div>
              );
            })}
        ...
        <Card className="glass-card p-4 mb-4 shadow-lg overflow-hidden" style={{ minHeight: '340px' }}>
          ...
          <div className="d-flex flex-column gap-2 overflow-auto pe-1" style={{ maxHeight: '250px' }}>
            {cloudFiles.length === 0 ? (
              ...
              cloudFiles.map(file => (
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} key={file.id} className="d-flex align-items-center justify-content-between p-3 bg-white bg-opacity-5 rounded-4 border border-white border-opacity-5 shadow-sm mb-1 hover-lift">
              ...
              >
                 ...
                 <div className="d-flex flex-column gap-2 overflow-auto pe-1" style={{ maxHeight: '200px' }}>
                    {incomingFiles.map(file => (
                      <div key={file.id} className="p-3 bg-white bg-opacity-5 rounded-4 border border-white border-opacity-5 d-flex align-items-center justify-content-between mb-1 hover-lift">
                        <div className="d-flex align-items-center gap-3 overflow-hidden">
                          {file.completed ? <CheckCircle2 size={18} className="text-cyan pulse" /> : <Spinner animation="border" size="sm" className="text-secondary opacity-50" style={{width: '14px', height: '14px'}} />}
                          <div className="text-start overflow-hidden">
                            <div className="fw-bold smaller text-truncate" style={{maxWidth: '120px'}}>{file.name}</div>
        ...
        <Card className="glass-card h-100 d-flex flex-column p-4 overflow-hidden shadow-lg border-white border-opacity-5">
          ...
          <div className="flex-grow-1 overflow-auto mb-3 pe-2 chat-container" style={{ maxHeight: '500px' }}>
            {messages.length === 0 ? (
              ...
              <div className="d-flex flex-column gap-3">
                {messages.map((m, i) => (
                  <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }} key={i} className={`p-3 rounded-4 shadow-sm ${m.sender === userName ? 'bg-cobalt bg-opacity-20 ms-3 border border-cobalt border-opacity-10 shadow-lg' : 'bg-white bg-opacity-5 me-3 border border-white border-opacity-5'}`}>
                    <div className="smaller mono text-secondary mb-1 d-flex justify-content-between border-bottom border-white border-opacity-5 pb-1 mb-2" style={{ fontSize: '0.65rem' }}>
                      <span className="fw-bold text-cyan">{m.sender}</span>
          ...
          <Form onSubmit={onSendChat}>
            <InputGroup className="shadow-lg rounded-pill overflow-hidden">
              ...
              />
              <Button type="submit" variant="none" className="bg-cyan bg-opacity-10 text-cyan border border-white border-opacity-10 px-3 hover-glow">
                <Zap size={16} />
              </Button>
            </InputGroup>
          </Form>
